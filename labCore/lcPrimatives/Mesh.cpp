...
Node* Mesh::Clone()
{
	...
	return dynamic_cast<Node*>(pClone);
}

//--------------------------------------------------------------------------------//

void Mesh::SetPrimativeTopology(D3D11_PRIMITIVE_TOPOLOGY a_oTopology)
{
...
void Mesh::AttachProperty(Property* a_pProp)
{
	PROPERTY_TYPES ePropType = a_pProp->GetType();

	switch(ePropType)
	{
	case PROPERTY_SHADER:
		{
			ShaderProperty* pProp = dynamic_cast<ShaderProperty*>(a_pProp);
			if(pProp)
			{
				m_pInputLayout = pProp->GetInputLayout();
				m_pVertexShader = pProp->GetVertexShader();
				m_pPixelShader = pProp->GetPixelShader();
				m_aProperties.push_front(pProp);
			}
			break;
		}
...
bool Mesh::AttachVertexData(D3D11_BUFFER_DESC a_oVertexDesc,D3D11_SUBRESOURCE_DATA a_oVertexData,unsigned int a_uiVertexCount,unsigned int a_uiStride)
{
...
bool Mesh::AttachIndexData(D3D11_BUFFER_DESC a_oIndexDesc,D3D11_SUBRESOURCE_DATA a_oIndexData,unsigned int a_uiIndexCount,unsigned int a_uiOffset)
{
