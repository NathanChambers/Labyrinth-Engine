...
//--------------------------------------------------------------------------------//
DX11Renderer* DX11Renderer::m_pSingleton = 0;
//--------------------------------------------------------------------------------//
...
DX11Renderer* DX11Renderer::Create(Window* a_pWindow)
{
	if(!m_pSingleton)
	{
		m_pSingleton = new DX11Renderer();
...
void DX11Renderer::BeginFrame(D3DXCOLOR a_oColor)
{
...
bool DX11Renderer::Initialise(Window* a_pWindow)
{
	//++++++++++++++++++++++++++++++++++++++++//
...
void DX11Renderer::DrawScene(DX11Camera* a_pCamera,Node* a_pRootNode)
{
	//Update ViewProj
	D3DXMATRIX mViewProj;
...
void DX11Renderer::TraverseChildren(Node* a_pRootNode)
{
	if(a_pRootNode->GetChildCount() > 0)
	{
		for(unsigned int i = 0;i < a_pRootNode->GetChildCount();i++)
		{
			Node* pChild = a_pRootNode->GetAt(i);
...
void DX11Renderer::RenderNode(Node* a_pNode)
{
	Mesh* pMesh = dynamic_cast<Mesh*>(a_pNode);
	if(pMesh)
	{
		//Update World Matrix
